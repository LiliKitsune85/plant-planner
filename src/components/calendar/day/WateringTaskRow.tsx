...
interface WateringTaskRowProps {
...
const statusMeta: Record<CalendarDayTaskVm["status"], { label: string; className: string }> = {
  pending: {
    label: "Do wykonania",
      ...
      <article className="space-y-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0">
            <p className="text-base font-semibold text-foreground">{task.plantDisplayName}</p>
            {task.plantNickname && <p className="text-sm text-muted-foreground">{task.plantNickname}</p>}
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <span className={cn("rounded-full px-3 py-1 text-xs font-semibold", statusMeta[task.status].className)}>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button size="sm" variant="outline" onClick={handleEdit} disabled={isPending || !onEdit}>
            Edytuj
          </Button>

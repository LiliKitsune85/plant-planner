...
const mapConfigToVm = (dto: WateringPlanSuggestionDto): AiSuggestionAvailableVm | AiSuggestionSkippedVm => {
  if (!dto.suggestion) {
    return {
      status: "skipped",
...
export const normalizeSpeciesNameForSuggest = (value?: string | null): string | null => clampSpeciesName(value);
...
export const mapSuggestionDtoToVm = (dto: WateringPlanSuggestionDto): AiSuggestionAvailableVm | AiSuggestionSkippedVm =>
  mapConfigToVm(dto);

export const mapCreationSuggestionToState = (suggestion: WateringSuggestionForCreationDto): AiSuggestionStateVm => {
...
export const buildRateLimitedVm = (error: SuggestWateringPlanApiError): AiSuggestionRateLimitedVm => ({
  status: "rate_limited",
  unlockAt: parseUnlockAt(error.details),
  aiRequestId: error.suggestion?.ai_request_id ?? null,
});

export const buildSuggestionErrorVm = (error: SuggestWateringPlanApiError): AiSuggestionErrorVm => {
  switch (error.kind) {
    case "timeout":
      return {
        status: "timeout",
        code: error.code,
        message: error.message ?? "AI nie odpowiedziało na czas. Spróbuj ponownie lub ustaw plan ręcznie.",
    ...
    case "provider_error":
      return {
        status: "provider_error",
        code: error.code,
        message: error.message ?? "Dostawca AI zwrócił błąd. Spróbuj ponownie później lub ustaw plan ręcznie.",
        requestId: error.requestId,
        details: error.details,
      };
...
export const formValuesFromSuggestion = (suggestion: AiSuggestionAvailableVm): WateringPlanFormValues => ({
...
export const formValuesFromConfig = (config: WateringPlanConfigFields): WateringPlanFormValues => ({
...
export const sanitizeFormToSetCommand = (
  ...
  const intervalDays = ensureRange(parseNumber(values.interval_days, DEFAULT_INTERVAL_DAYS), 1, 365);
  const horizonDays = ensureRange(parseNumber(values.horizon_days, DEFAULT_HORIZON_DAYS), 1, 365);
  const startFrom = values.start_from;
  const customStartOn = startFrom === "custom_date" && values.custom_start_on ? values.custom_start_on : null;
...
export const validateWateringPlanForm = (values: WateringPlanFormValues): WateringPlanFormErrors => {
...
export const buildSetPlanErrorVm = (error: SetWateringPlanApiError): SetPlanErrorVm => {
  ...
  if (error.kind === "conflict") {
    return {
      kind: "conflict",
      message: error.message ?? "Plan został zmieniony w tle. Odśwież dane i spróbuj ponownie ustawić plan.",
      code: error.code,
      requestId: error.requestId,
      details: error.details,
    };

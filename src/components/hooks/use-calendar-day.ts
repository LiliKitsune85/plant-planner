...
interface UseCalendarDayParams {
  date: string;
...
export const invalidateCalendarDayCache = (predicate?: (key: string, value: CalendarDayVm) => boolean): void => {
  if (!predicate) {
    cache.clear();
    return;
  }
...
export const useCalendarDay = ({
  ...
  useEffect(() => {
    ...
    const fetchDay = async () => {
      try {
        const { data, requestId } = await getCalendarDay({ date, status, sort, order }, { signal: controller.signal });
        ...
        logger.error("Unexpected error in useCalendarDay", error);
        setState({
          status: "error",
          error: {
            kind: "unknown",

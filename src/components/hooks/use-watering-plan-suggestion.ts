...
import {
  suggestWateringPlan,
  SuggestWateringPlanApiError,
} from '@/lib/services/watering-plans/suggest-client'
import type { WateringSuggestionForCreationDto } from '@/types'

type UseWateringPlanSuggestionParams = {
  plantId: string
...
type UseWateringPlanSuggestionResult = {
...
export const useWateringPlanSuggestion = ({
  ...
  const [state, setState] = useState<AiSuggestionStateVm>(() =>
    deriveInitialState(initialSuggestion),
  )
  ...
  const normalizedSpeciesName = useMemo(
    () => normalizeSpeciesNameForSuggest(speciesName),
    [speciesName],
  )
  ...
  const run = useCallback(() => {
    ...
    if (!enabled) {
      setState(
        buildSkippedState('Sugestie AI są obecnie wyłączone. Ustaw plan ręcznie, aby kontynuować.'),
      )
      return
    }
    ...
    void (async () => {
      try {
        const { data, requestId: reqId, responseTimeBudgetMs: budget } = await suggestWateringPlan(
          plantId,
          command,
        ...
        console.error('Unexpected error while requesting watering plan suggestion', error)
        setState(
          buildSuggestionErrorVm(

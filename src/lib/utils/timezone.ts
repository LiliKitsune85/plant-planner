...
export const formatDateInTimezone = (date: Date, timezone?: string): string => {
  ...
  try {
    ...
    console.error('formatDateInTimezone failed, falling back to UTC', { error, safeTimezone })
    return zonedFormatter('UTC').format(date)
  }
}

export const getTodayIsoDateInTimezone = (timezone?: string): string =>
  formatDateInTimezone(new Date(), timezone)

export const getTodayMonthInTimezone = (timezone?: string): string =>
  getTodayIsoDateInTimezone(timezone).slice(0, 7)

...
interface WateringPlanApiErrorOptions {
  status?: number;
  requestId?: string;
  details?: unknown;
...
export interface SetWateringPlanOptions {
  signal?: AbortSignal;
}

export interface SetWateringPlanResult {
  data: SetWateringPlanResultDto;
  requestId?: string;
}

const isValidSource = (source: unknown): source is SetWateringPlanCommand["source"] => {
  if (!source || typeof source !== "object") return false;
  const value = source as { type?: unknown };
  return value.type === "manual" || value.type === "ai";
};

const ensureManualSource = (command: SetWateringPlanCommand): SetWateringPlanCommand => ({
  ...command,
  source: { type: "manual" },
});

const ensureCommandHasSource = (command: SetWateringPlanCommand): SetWateringPlanCommand => {
  const candidate = (command as { source?: unknown }).source;
  if (isValidSource(candidate)) {
    return command;
  }

  logger.warn("setWateringPlan: payload missing source, defaulting to manual source.", { command });
...
export const setWateringPlan = async (
  ...
  try {
    ...
    throw new WateringPlanApiError("PARSE_ERROR", "Nie udało się przetworzyć odpowiedzi serwera planu podlewania.", {
  ...
  if (!response.ok || envelope.error || !envelope.data) {
    ...
    const message = envelope.error?.message ?? `Zapis planu nie powiódł się (status ${response.status}).`;

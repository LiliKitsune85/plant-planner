...
const removeFieldErrorsForPatch = (
  ...
  return Object.fromEntries(
    Object.entries(prevErrors).filter(([key]) => !keysToClear.has(key))
  ) as CreatePlantFormErrors["fieldErrors"];
};

interface UseCreatePlantResult {
...
export const useCreatePlant = (): UseCreatePlantResult => {
  ...
  const applyQuota = useCallback(
    (quota: AiQuotaDto) => {
      setLimitText(`Limit: ${quota.limit_per_hour} zapytań/h · Pozostało ${Math.max(0, quota.remaining)} zapytań`);
      ...
      if (quota.is_rate_limited && quota.unlock_at) {
        ...
        clearUnlockAt();
        setAiRateLimited((prev) => (prev.isRateLimited ? { isRateLimited: false, unlockAt: null } : prev));
      }
    },
    [setValue]
  );
  ...
  const refreshAiQuota = useCallback(
    async (signal?: AbortSignal) => {
      try {
        ...
        logger.warn("Failed to fetch AI quota", error);
      }
    },
    [applyQuota]
  );
  ...
  const handleChange = useCallback((patch: Partial<CreatePlantFormValues>) => {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (error instanceof PlantsApiError) {
        logger.error("createPlant request failed", {
          code: error.code,
          kind: error.kind,
          requestId: error.requestId,
          details: error.details,
        ...
        logger.error("createPlant request failed with unexpected error", error);

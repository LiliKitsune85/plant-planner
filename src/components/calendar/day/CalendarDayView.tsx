...
import type { CalendarTaskSortField, CalendarTaskStatusFilter, SortOrder } from "@/lib/services/calendar/types";
import {
...
export const CalendarDayView = ({ date, status, sort, order }: CalendarDayViewProps) => {
  const {
    status: fetchStatus,
    data,
    error,
  ...
  useEffect(() => {
    if (typeof window === "undefined") return;
    setLoginHref(`/auth/login?returnTo=${encodeURIComponent(`${window.location.pathname}${window.location.search}`)}`);
  }, []);
  ...
  useEffect(() => {
    if (mutations.error?.code === "TASK_ALREADY_EXISTS" && mutations.error.plantId) {
  ...
  const mergedTasks = useMemo(() => {
    ...
    return tasks.map((task) => mutations.optimisticTasks[task.id] ?? task);
  }, [data?.items, mutations.optimisticTasks]);
  const mergedTaskIdsKey = useMemo(() => mergedTasks.map((task) => task.id).join("|"), [mergedTasks]);
  ...
  const editError =
    editingTaskId && activeError?.taskId === editingTaskId && activeError.kind === "validation" ? activeError : null;
  const adhocDialogError = adhocOpen && activeError?.plantId ? activeError : null;
  const suppressInline =
    (editError && activeError === editError) ||
    (adhocDialogError && activeError === adhocDialogError && activeError.kind === "validation");
  ...
  const mutationAlert = useMemo(() => {
    ...
    if (inlineError.kind === "conflict" && inlineError.code === "TASK_ALREADY_EXISTS" && inlineError.plantId) {
      return {
  ...
  const handleAdhocSubmit = async (plantId: string, command: Parameters<typeof mutations.createAdhoc>[1]) => {
  ...
  return (
    <main className="mx-auto max-w-3xl space-y-6 p-4 sm:p-6">
      ...
      {mutationAlert && (
        <div role="status" className={`rounded-xl border p-4 text-sm ${mutationAlertClasses[mutationAlert.variant]}`}>
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              {mutationAlert.primary && (
                <Button size="sm" variant="outline" onClick={handleAlertAction(mutationAlert.primary.handler)}>
      ...
      {editingTask && (
        <EditWateringEntryDialog
          ...
          error={editError ? { message: editError.message, fieldErrors: editError.fieldErrors } : null}
          onOpenChange={(open) => {
      ...
      <AdhocWateringDialog
        ...
        error={
          adhocDialogError ? { message: adhocDialogError.message, fieldErrors: adhocDialogError.fieldErrors } : null
        }

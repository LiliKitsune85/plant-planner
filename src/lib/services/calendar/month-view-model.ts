...
import type { CalendarTaskStatusFilter, CalendarTaskSortField, SortOrder } from "./types";
...
export interface CalendarMonthGridDayVm {
  date: string;
  dayNumber: number;
  count: number;
...
export interface CalendarMonthGridWeekVm {
  days: CalendarMonthGridDayVm[];
}

export interface CalendarMonthGridVm {
  month: string;
  weeks: CalendarMonthGridWeekVm[];
  weekdayLabels: string[];
}

export interface CalendarMonthErrorVm {
...
export interface MonthPickerVmLink {
  label: string;
  month: string;
  href: string;
}

export interface MonthPickerVm {
  currentMonth: string;
  currentLabel: string;
...
export interface CalendarStatusFilterOption {
...
export const CALENDAR_STATUS_VALUES: CalendarTaskStatusFilter[] = ["pending", "completed", "all"] as const;
...
const weekdayFormatter = (locale: string) => new Intl.DateTimeFormat(locale, { weekday: "short" });
...
const parseCalendarMonth = (month: string): { year: number; monthIndex: number } | null => {
...
export const defaultMonthHrefBuilder = (month: string, status: CalendarTaskStatusFilter): string => {
...
const buildAriaLabel = (date: string, count: number, status: CalendarTaskStatusFilter): string => {
...
export const isValidCalendarMonthString = (value: string): boolean => calendarMonthRegex.test(value);
...
): CalendarTaskStatusFilter => (status && CALENDAR_STATUS_VALUES.includes(status) ? status : defaultValue);
...
export const getCalendarMonthCacheKey = (month: string, status: CalendarTaskStatusFilter): string =>
  `${month}:${status}`;
...
interface BuildCalendarMonthGridOptions {
  locale?: string;
  todayIsoDate?: string;
...
export const buildCalendarMonthGridVm = (
  ...
  for (let weekIndex = 0; weekIndex < 6; weekIndex++) {
    ...
    for (let dayIndex = 0; dayIndex < 7; dayIndex++) {
      ...
      const isInCurrentMonth = cellDate.getFullYear() === parsed.year && cellDate.getMonth() === parsed.monthIndex;
...
interface BuildMonthPickerVmOptions {
  locale?: string;
  todayMonth?: string;
  monthHrefBuilder?: (month: string, status: CalendarTaskStatusFilter) => string;
}

export const buildMonthPickerVm = (vm: CalendarMonthVm, options: BuildMonthPickerVmOptions = {}): MonthPickerVm => {
  const locale = options.locale ?? defaultLocale;
  ...
  const currentLabel = monthFormatter(locale).format(new Date(`${vm.month}-01T00:00:00.000Z`));
  ...
  return {
    ...
    today: {
      label: monthFormatter(locale).format(new Date(`${todayMonth}-01T00:00:00.000Z`)),
      month: todayMonth,
...
interface BuildCalendarMonthStatusFilterOptions {
  monthHrefBuilder?: (month: string, status: CalendarTaskStatusFilter) => string;
}
...
export const buildCalendarMonthStatusFilterOptions = (
  ...
  const monthHrefBuilder = options.monthHrefBuilder ?? defaultMonthHrefBuilder;
  return buildStatusFilterOptions(vm.status, (status) => monthHrefBuilder(vm.month, status));
};

export const buildCalendarMonthErrorVm = (error: CalendarMonthApiError): CalendarMonthErrorVm => ({
  kind: error.kind,
...
export const buildClientValidationErrorVm = (month: string): CalendarMonthErrorVm => ({
  kind: "validation",

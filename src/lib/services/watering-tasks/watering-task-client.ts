...
type WateringTaskApiErrorOptions = {
  status?: number
  requestId?: string
  details?: unknown
...
type UpdateWateringTaskOptions = {
  signal?: AbortSignal
}
...
type DeleteWateringTaskOptions = {
  signal?: AbortSignal
}
...
export const deleteWateringTask = async (
  ...
  requestWateringTaskApi<DeleteWateringTaskResultDto>(
    `/api/watering-tasks/${taskId}?confirm=true`,
    {
      method: 'DELETE',
      signal: options.signal,
    },

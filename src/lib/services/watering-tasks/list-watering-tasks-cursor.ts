...
const isSortField = (value: unknown): value is WateringTaskSortField =>
  value === 'due_on' || value === 'created_at'

const isSortOrder = (value: unknown): value is WateringTaskSortOrder =>
...
const isFiltersSnapshot = (
  value: unknown,
): value is ListWateringTasksCursorFiltersSnapshot => {
...
export const encodeListWateringTasksCursor = (
  payload: ListWateringTasksCursorPayload,
): string => {
...
export const decodeListWateringTasksCursor = (
  ...
  const { version: _version, ...payload } = envelope
  return payload
}

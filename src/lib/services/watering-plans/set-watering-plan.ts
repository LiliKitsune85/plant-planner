...
interface RegenerateTasksRpcArgs {
  p_user_id: string;
  p_plant_id: string;
  p_plan_id: string;
  p_interval_days: number;
...
interface RegenerateTasksRpcRow {
...
const ensurePlantOwnership = async (supabase: SupabaseClient, userId: string, plantId: string): Promise<void> => {
  ...
  if (error) {
    logger.error("setPlantWateringPlan: plant lookup failed", { error, userId, plantId });
...
const ensureAiRequestOwnership = async (
  ...
  if (error) {
    logger.error("setPlantWateringPlan: ai request lookup failed", { error, userId, aiRequestId });
...
const buildSetPlanPayload = (plantId: string, userId: string, command: SetWateringPlanCommand): SetPlanRpcArgs => {
  ...
  return {
    ...
    p_was_ai_accepted_without_changes: isAiSource ? command.source.accepted_without_changes : null,
    p_ai_request_id: isAiSource ? command.source.ai_request_id : null,
  };
};

const buildRegeneratePayload = (plan: WateringPlanRow, userId: string): RegenerateTasksRpcArgs => ({
  p_user_id: userId,
...
interface SetPlantWateringPlanParams {
  userId: string;
  plantId: string;
...
export const setPlantWateringPlan = async (
  ...
  const { data: planRow, error: setPlanError } = await supabase.rpc("set_watering_plan_version", rpcPayload).single();
  ...
  if (setPlanError || !planRow) {
    ...
    logger.error("setPlantWateringPlan: plan RPC failed", {
      error: setPlanError,
      userId,
      plantId,
    });
  ...
  const { data: regenerateRow, error: regenerateError } = await supabase
    .rpc("regenerate_watering_tasks", regeneratePayload)
    .single();

  if (regenerateError || !regenerateRow) {
    logger.error("setPlantWateringPlan: task regeneration failed", {

...
const schema = z
  .object({
    ...
    confirmPassword: z.string().min(8, "Pole wymagane.").max(160, "Hasło jest za długie."),
...
export const SignUpForm = ({ returnTo = "/calendar" }: SignUpFormProps) => {
  ...
  const handleInputChange = useCallback(
    (field: "email" | "password" | "confirmPassword" | "nickname") => (event: ChangeEvent<HTMLInputElement>) =>
  ...
  return (
    <form className="space-y-5" onSubmit={onSubmit} noValidate>
      {errors.form ? <AuthErrorAlert title="Nie udało się utworzyć konta" description={errors.form} /> : null}

      {successMessage ? (
        <AuthNotice tone="success" title="Sprawdź skrzynkę" description={successMessage}>
      ...
      <div className="space-y-2">
        ...
        <Input
          ...
          aria-describedby={fieldErrors.confirmPassword?.length ? "sign-up-password-confirm-errors" : undefined}
          disabled={isSubmitting}
        />
      ...
      <div className="rounded-lg border border-dashed border-border/80 bg-muted/40 px-4 py-3 text-sm text-muted-foreground">
        <p>
          Wykryta strefa czasowa: <span className="font-medium text-foreground">{values.timezone ?? "UTC"}</span>.
          Użyjemy jej, aby prawidłowo wyliczać przypomnienia o podlewaniu.
        </p>
      </div>

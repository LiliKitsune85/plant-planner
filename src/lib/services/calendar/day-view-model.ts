...
export interface CalendarDayTaskVm {
  id: string;
  plantId: string;
...
export interface CalendarDayHeaderVm {
  date: string;
  dateLabel: string;
  weekdayLabel: string;
  monthHref: string;
...
export interface CalendarDaySortOption {
...
export interface CalendarDayOrderOption {
  value: SortOrder;
  label: string;
  href: string;
  isActive: boolean;
...
const toTaskSource = (value: string): CalendarDayTaskSource => (value === "adhoc" ? "adhoc" : "scheduled");
...
export const buildCalendarDayStatusFilterOptions = (
  ...
  buildStatusFilterOptions(vm.status, (status) => dayHrefBuilder(vm.date, status, vm.sort, vm.order));
...
export const buildCalendarDayErrorVm = (error: CalendarDayApiError): CalendarDayErrorVm => ({
  kind: error.kind,
...
export const normalizeCalendarDayStatus = (status?: CalendarTaskStatusFilter): CalendarTaskStatusFilter =>
  normalizeCalendarStatusFilter(status, "pending");

const sortValues: CalendarTaskSortField[] = ["species_name", "due_on"];
export const normalizeCalendarDaySort = (sort?: CalendarTaskSortField): CalendarTaskSortField =>
...
export const getCalendarStatusLabel = (status: CalendarTaskStatusFilter): string => CALENDAR_STATUS_LABELS[status];

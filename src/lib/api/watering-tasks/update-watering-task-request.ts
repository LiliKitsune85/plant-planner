...
const UpdateWateringTaskPayloadSchema = z
  ...
  .superRefine((value, ctx) => {
    const hasAnyField = value.status !== undefined || value.completed_on !== undefined || value.note !== undefined;
...
export const parseUpdateWateringTaskParams = (params: Record<string, string | undefined>): UpdateWateringTaskParams => {
...
export const parseUpdateWateringTaskRequest = (body: unknown): UpdateWateringTaskCommand => {
  ...
  const sanitizedEntries = Object.entries(parsed.data).filter(([, value]) => value !== undefined) as [
    keyof UpdateWateringTaskCommand,
    UpdateWateringTaskCommand[keyof UpdateWateringTaskCommand],
  ][];

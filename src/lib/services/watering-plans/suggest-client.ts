...
type SuggestWateringPlanApiErrorOptions = {
  status?: number
  requestId?: string
  details?: unknown
...
const determineErrorKind = (
  status?: number,
  code?: string,
): SuggestWateringPlanApiErrorKind => {
  switch (code) {
...
type RequestSuggestWateringPlanResult = {
  data: WateringPlanSuggestionDto
  requestId?: string
  responseTimeBudgetMs?: number
}

type SuggestWateringPlanOptions = {
  signal?: AbortSignal
}
...
export const suggestWateringPlan = async (
  ...
  try {
    ...
    throw new SuggestWateringPlanApiError(
      'NETWORK_ERROR',
      'Nie udało się połączyć z usługą sugestii planu.',
      {
        kind: 'network',
        cause: error,
      },
  ...
  try {
    ...
    throw new SuggestWateringPlanApiError(
      'PARSE_ERROR',
      'Nie udało się przetworzyć odpowiedzi z usługi AI.',
      {
  ...
  if (!response.ok || envelope.error || !envelope.data) {
    ...
    const message =
      envelope.error?.message ?? `Sugestia AI nie powiodła się (status ${response.status}).`

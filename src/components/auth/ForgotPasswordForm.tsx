...
export const ForgotPasswordForm = () => {
  ...
  return (
    <form className="space-y-5" onSubmit={onSubmit} noValidate>
      {errors.form ? <AuthErrorAlert title="Nie udało się wysłać wiadomości" description={errors.form} /> : null}

      {successMessage ? (
        <AuthNotice tone="success" title="Jeśli konto istnieje, wyślemy e-mail" description={successMessage}>
          <p className="text-sm text-emerald-900/80">
            Ze względów bezpieczeństwa komunikat jest zawsze taki sam, niezależnie od tego, czy adres jest
            zarejestrowany.
          </p>
        </AuthNotice>
      ) : null}

...
const getNextDuplicateIndex = async (
  ...
  if (error) {
    logger.error("createPlant duplicate_index lookup failed", {
      error,
      userId,
      speciesNameNormalized,
    });
    throw new HttpError(500, "Failed to compute plant duplicate index", "PLANT_DUPLICATE_INDEX_LOOKUP_FAILED");
  }
...
export interface CreatePlantServiceCommand {
  userId: string;
...
export const createPlant = async (
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    const { data, error } = await supabase.from("plants").insert(insert).select(INSERT_RESULT_COLUMNS).single();
    ...
    if (error) {
      ...
      if (isUniqueViolation(error)) {
        throw new HttpError(409, "Duplicate index conflict, retry later", "DUPLICATE_INDEX_CONFLICT");
      }

      logger.error("createPlant insert failed", { error, userId });
      throw new HttpError(500, "Failed to create plant", "PLANT_CREATE_FAILED");
    }

    if (!data) {
      logger.error("createPlant insert returned no data", { userId });
      throw new HttpError(500, "Failed to create plant", "PLANT_CREATE_FAILED");
    }

...
export const loadActivePlanForPlant = async (
  ...
  if (error) {
    logger.error("planRegeneration: failed to load active plan", {
      error,
      userId,
      plantId,
...
export const regenerateTasksForPlan = async (
  ...
  if (plan.interval_days === null || plan.interval_days === undefined || plan.schedule_basis === null) {
    logger.error("planRegeneration: plan missing scheduling fields", {
      planId: plan.id,
      userId,
      requestId: context?.requestId,
    });
  ...
  if (error || !data) {
    logger.error("planRegeneration: task regeneration failed", {
      error,
      userId,
      planId: plan.id,
      requestId: context?.requestId,

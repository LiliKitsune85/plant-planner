...
export type CalendarMonthApiErrorKind = "validation" | "unauthenticated" | "http" | "network" | "parse" | "unknown";

interface CalendarMonthApiErrorOptions {
  status?: number;
  requestId?: string;
  details?: unknown;
...
const determineErrorKind = (status?: number, code?: string): CalendarMonthApiErrorKind => {
...
interface GetCalendarMonthParams {
  month: string;
  status?: CalendarTaskStatusFilter;
}

interface GetCalendarMonthOptions {
  signal?: AbortSignal;
}

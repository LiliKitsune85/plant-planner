...
type CreatePlantInlineErrorProps = {
...
export const CreatePlantInlineError = ({
  error,
  onRetry,
  onDismiss,
  loginHref,
}: CreatePlantInlineErrorProps) => {
  const showRetry =
    error.kind === 'network' ||
    error.kind === 'conflict' ||
    error.kind === 'http' ||
  ...
  return (
    ...
    >
      ...
      <CardContent className="space-y-3">
        {error.requestId ? (
          <p className="text-xs text-muted-foreground">ID żądania: {error.requestId}</p>
        ) : null}
        {showActions ? (

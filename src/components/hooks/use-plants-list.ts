...
export type UsePlantsListResult = {
...
export const usePlantsList = ({ initialQuery }: UsePlantsListOptions): UsePlantsListResult => {
  const [query, setQueryState] = useState<PlantsListQueryState>(() =>
    normalizePlantsListQueryState(initialQuery),
  )
  ...
  useEffect(() => {
    ...
    const fetchPlants = async () => {
      try {
        const { data, nextCursor: cursor, requestId: reqId } = await listPlants(
          {
            q: query.q,
        ...
        if (err instanceof PlantsApiError) {
          ...
          console.error('Unexpected error in usePlantsList', err)
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      if (err instanceof PlantsApiError) {
        ...
        console.error('Unexpected error while loading more plants', err)
        setLoadMoreError(
          buildUnknownPlantsListErrorVm('Nie udało się pobrać kolejnej strony roślin.'),
        )
      }
    } finally {
  ...
  const isFiltered = Boolean(
    (query.q && query.q.length > 0) || (query.species && query.species.length > 0),
  )

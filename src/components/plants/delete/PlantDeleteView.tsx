...
export interface PlantDeleteViewProps {
  plantId: string;
}
...
export const PlantDeleteView: FC<PlantDeleteViewProps> = ({ plantId }) => {
  const { status, data, error: loadError, requestId: loadRequestId, reload } = usePlantDelete({ plantId });
  const { pending, error: mutationError, deletePlant, resetError, cancel } = useDeletePlantMutation({ plantId });
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      window.location.assign(successState.redirectHref);
    }, 2200);
      ...
      <Card className="border-emerald-400/40 bg-emerald-50 text-emerald-900">
        <CardHeader>
          ...
          <CardDescription className="text-sm text-emerald-800/90">
            „{successState.plantName}” została usunięta wraz z harmonogramem. Za chwilę przekierujemy Cię na listę
            roślin.
            {successState.requestId ? (
              <span className="mt-2 block text-xs text-emerald-800/70">ID żądania: {successState.requestId}</span>
            ) : null}
          </CardDescription>
        </CardHeader>
    ...
    <main className="mx-auto max-w-2xl space-y-6 p-4 sm:p-6" aria-live="polite">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Operacja natychmiast usuwa roślinę oraz wszystkie jej zadania i plan podlewania. Potwierdź, że rozumiesz
            konsekwencje, zanim przejdziesz dalej.
          </CardDescription>
        </CardHeader>
        <CardContent className="text-sm text-muted-foreground">
          ...
          {loadRequestId ? <p className="mt-2 text-xs text-muted-foreground/80">ID żądania: {loadRequestId}</p> : null}
        </CardContent>
        <CardFooter className="flex flex-wrap gap-3">
          <Button variant="outline" asChild>
      ...
      {status === "error" && loadError ? (
        <PlantDeleteErrorState error={loadError} onRetry={reload} onBack={navigateBack} loginHref={loginHref} />
      ) : null}

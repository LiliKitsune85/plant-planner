...
export interface WateringPlanEditorViewProps {
  plantId: string;
  mode?: "manual" | "ai";
...
const validateFormState = (state: WateringPlanFormState): WateringPlanFormErrors | null => {
  ...
  errors.form = ["Formularz zawiera błędy. Popraw zaznaczone pola i spróbuj ponownie."];

  return errors;
};

const buildModeContext = (mode: "manual" | "ai", aiRequestId?: string | null): WateringPlanEditorModeContext => {
...
const mapFormStateToCommand = (
  ...
  const customStartOn = startFrom === "custom_date" && state.customStartOn ? state.customStartOn : null;
...
export const WateringPlanEditorView: FC<WateringPlanEditorViewProps> = ({
  ...
  const { pending, error, submitSetPlan, clearError, lastResult } = useWateringPlanMutations();

  const modeContext = useMemo(() => buildModeContext(mode, aiRequestId), [mode, aiRequestId]);
  const validationSnapshot = useMemo(() => validateFormState(formState), [formState]);
  ...
  const clearFieldErrors = useCallback(
    (fields: (keyof WateringPlanFormErrors)[]) => {
      setFormErrors((prev) => {
        if (!prev) return prev;
        const keysToClear = new Set(fields as string[]);
        const next = Object.fromEntries(
          Object.entries(prev).filter(([key]) => !keysToClear.has(key))
        ) as WateringPlanFormErrors;
  ...
  const handleAdvancedChange = useCallback(
    (patch: Partial<WateringPlanAdvancedState>) => {
      const fieldsToClear: (keyof WateringPlanFormErrors)[] = [];
  ...
  }, [clearError, formState, isBusy, isPlantIdValid, modeContext, plantId, submitSetPlan, validationSnapshot]);

  const blockingError: WateringPlanMutationError | null = error && error.kind !== "validation" ? error : null;
      ...
      <main className="mx-auto max-w-3xl space-y-6 p-4 sm:p-6">
        ...
        <Card className="border-destructive/40 bg-destructive/5">
          <CardHeader>
            ...
            <CardDescription>
              Przekazany identyfikator rośliny jest pusty lub ma niepoprawny format. Wróć do listy roślin i spróbuj
              ponownie.
            </CardDescription>
          </CardHeader>
          <CardContent>
  ...
  return (
    <main className="mx-auto max-w-3xl space-y-6 p-4 sm:p-6">
      <WateringPlanEditorHeader backHref={`/plants/${plantId}/watering-plan`} onBack={handleCancel} />
      ...
      <section className="rounded-3xl border border-border/70 bg-muted/20 p-4 text-sm text-muted-foreground">
        ...
        <p>
          Określ co ile dni należy podlewać roślinę oraz wybierz datę rozpoczęcia harmonogramu. Po zapisaniu planu
          kalendarz zostanie zaktualizowany automatycznie.
        </p>
      </section>
      ...
      {lastResult ? <TasksRegeneratedSummaryCard summary={lastResult.tasks_regenerated} /> : null}
    </main>
  );
};

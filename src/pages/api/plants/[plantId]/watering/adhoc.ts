...
export const POST: APIRoute = async ({ locals, params, request }) => {
  ...
  try {
    ...
    return json(201, { data, error: null, meta: { request_id: requestId } }, { Location: location });
  } catch (error) {
    if (isHttpError(error)) {
      logger.error("Handled error in POST /api/plants/[plantId]/watering/adhoc", {
        error,
        requestId,
    ...
    logger.error("Unhandled error in POST /api/plants/[plantId]/watering/adhoc", {
      error,
      requestId,
      plantId: params?.plantId,
    });

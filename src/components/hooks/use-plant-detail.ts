...
import type { PlantDetailErrorVm, PlantDetailVm, PlantDetailStatus } from "@/components/plants/detail/types";
import {
...
export interface PlantDetailInitialState {
...
interface UsePlantDetailParams {
  plantId: string;
...
export const usePlantDetail = ({ plantId, initial }: UsePlantDetailParams): UsePlantDetailResult => {
  ...
  useEffect(() => {
    ...
    const fetchDetail = async () => {
      try {
        const { data, requestId: reqId } = await getPlantDetail({ plantId }, { signal: controller.signal });
        ...
        if (err instanceof PlantsApiError) {
          ...
          logger.error("Unexpected error while loading plant detail", err);

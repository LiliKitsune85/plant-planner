...
export const ActivePlanCard: FC<ActivePlanCardProps> = ({ plan, changePlanHref }) => (
  <div className="space-y-4">
    <div className="space-y-1">
      ...
      <h3 className="text-2xl font-semibold text-foreground">Podlewaj {intervalLabel(plan.intervalDays)}</h3>
      ...
      <p className="text-sm text-muted-foreground">Zaległości: {overduePolicyLabels[plan.overduePolicy]}</p>
      ...
      {plan.wasAiSuggested ? (
        <span className="inline-flex items-center gap-2 rounded-full border border-primary/30 bg-primary/5 px-3 py-1 text-xs font-medium text-primary">
          AI
          {plan.wasAiAcceptedWithoutChanges ? "zaakceptowano bez zmian" : "dostosowano przed zapisem"}
        </span>
      ) : null}
    </div>

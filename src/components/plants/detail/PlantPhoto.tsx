...
const isProbablyUrl = (value: string | null): boolean => Boolean(value && /^https?:\/\//i.test(value));
  ...
  if (photoPath && isProbablyUrl(photoPath)) {
    return (
      <img
        src={photoPath}
        alt={alt}
        className={cn(sizeClass, "rounded-2xl border border-border object-cover shadow-sm ring-1 ring-border/50")}
      />
    );
  }

...
type WateringPlanConfigFields = Pick<
  WateringPlanRow,
  "interval_days" | "horizon_days" | "schedule_basis" | "start_from" | "custom_start_on" | "overdue_policy"
>;
...
type WateringPlanStateFields = Pick<WateringPlanRow, "id" | "is_active" | "valid_from" | "valid_to">;

type WateringPlanSummaryFields = WateringPlanStateFields & WateringPlanConfigFields & WateringPlanAiMetaFields;
...
export interface PaginatedListDto<TItem> {
  items: TItem[];
}

export interface DeletionResultDto<IdType> {
  deleted: true;
  target_id?: IdType;
}
...
export type UpdateProfileCommand = Partial<Pick<ProfileRow, "nickname" | "timezone">>;
...
interface WateringSuggestionBase {
...
export interface PlantPhotoUploadCommand {
  content_type: string;
  filename: string;
}

export interface PlantPhotoUploadResultDto {
  upload: {
    method: "PUT";
    url: string;
...
export interface SuggestWateringPlanCommand {
  context: Pick<PlantRow, "species_name">;
}

export interface WateringPlanSuggestionDto {
  ai_request_id: AiRequestRow["id"];
...
export interface TasksRegeneratedSummary {
  from: IsoDate;
  to: IsoDate;
  count: number;
}

export interface SetWateringPlanResultDto {
...
export interface CalendarTaskSummaryDto {
...
export type UpdateWateringTaskCommand = Partial<Pick<WateringTaskRow, "status" | "completed_on" | "note">>;

export interface ScheduleEffectDto {
  tasks_regenerated: boolean;
  reason: string | null;
}

export interface UpdateWateringTaskResultDto {
  task: WateringTaskSummaryFields;
  schedule_effect: ScheduleEffectDto;
}

export type AdhocWateringCommand = Pick<WateringTaskRow, "completed_on" | "note">;

export interface AdhocWateringResultDto {
  task: WateringTaskSummaryFields & Pick<WateringTaskRow, "plant_id">;
}

export type DeleteWateringTaskResultDto = DeletionResultDto<WateringTaskRow["id"]> & {
  task_id: WateringTaskRow["id"];
};

export interface AccountDeleteIntentCommand {
  password: string;
  confirmation: string;
}

export interface AccountDeleteIntentResultDto {
  intent_id: string;

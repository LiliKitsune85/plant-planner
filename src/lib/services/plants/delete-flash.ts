...
export interface PlantsFlashMessage {
  kind: "plantDeleted";
  plantName: string;
  requestId?: string;
  createdAt: number;
...
const isPlantsFlashMessage = (value: unknown): value is PlantsFlashMessage => {
  ...
  return record.kind === "plantDeleted" && typeof record.plantName === "string" && typeof record.createdAt === "number";
};

export const savePlantDeletedFlashMessage = (params: { plantName: string; requestId?: string }): void => {
  ...
  try {
    ...
    logger.warn("Failed to persist plants flash message", error);
  }
};
...
export const consumePlantsFlashMessage = (): PlantsFlashMessage | null => {
  ...
  try {
    ...
    logger.warn("Failed to read plants flash message", error);
  }
  return null;
};

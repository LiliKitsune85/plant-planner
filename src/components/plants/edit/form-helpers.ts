...
import type { PlantEditDirtyState, PlantEditFieldKey, PlantEditFormErrors, PlantEditFormValues } from "./types";
...
const EDITABLE_FIELD_KEYS: (keyof UpdatePlantCommand)[] = ["nickname", "description", "purchase_date", "photo_path"];

const ISO_DATE_REGEX = /^\d{4}-\d{2}-\d{2}$/;

const ensureFieldError = (target: PlantEditFormErrors["fields"], field: PlantEditFieldKey, message: string) => {
...
const getComparablePayload = (values: PlantEditFormValues): Record<keyof UpdatePlantCommand, string | null> => ({
...
export const validatePlantEditValues = (values: PlantEditFormValues): PlantEditFormErrors => {
  ...
  if (purchaseDate && !isValidIsoDate(purchaseDate)) {
    ensureFieldError(fieldErrors, "purchase_date", "Data musi być w formacie RRRR-MM-DD i być poprawną datą.");
  }
  ...
  if (photoPath) {
    ...
    if (containsForbiddenSegments(photoPath)) {
      ensureFieldError(fieldErrors, "photo_path", "Ścieżka nie może zawierać znaków ?, #, \\, ani sekwencji ..");
    }
  }
...
export const calculatePlantEditDirtyState = (
  ...
  const changedFields = EDITABLE_FIELD_KEYS.filter((field) => initialPayload[field] !== currentPayload[field]);
...
const mapIssuePathToField = (path: (string | number)[] | undefined): PlantEditFieldKey => {
  if (!path || path.length === 0) return "form";
  const [first] = path;
  if (first === "nickname" || first === "description" || first === "purchase_date" || first === "photo_path") {
    return first;
  }
  return "form";
};
...
export const mergePlantEditFieldErrorsFromDetails = (
  ...
  if ("field" in details && typeof (details as FieldDetails).field === "string") {
    ...
    const message = typeof messageValue === "string" ? messageValue : "Pole zawiera nieprawidłowe dane.";

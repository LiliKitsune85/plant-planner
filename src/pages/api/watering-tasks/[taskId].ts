...
export const PATCH: APIRoute = async ({ locals, params, request }) => {
  ...
  try {
    ...
    if (isHttpError(error)) {
      logger.error("Handled error in PATCH /api/watering-tasks/[taskId]", {
        error,
        requestId,
    ...
    logger.error("Unhandled error in PATCH /api/watering-tasks/[taskId]", {
      error,
      requestId,
      taskId: params?.taskId,
    });
...
export const DELETE: APIRoute = async ({ locals, params, request, url }) => {
  ...
  try {
    ...
    if (isHttpError(error)) {
      logger.error("Handled error in DELETE /api/watering-tasks/[taskId]", {
        error,
        requestId,
        taskId: params?.taskId,
      });
    ...
    logger.error("Unhandled error in DELETE /api/watering-tasks/[taskId]", {
      error,
      requestId,
      taskId: params?.taskId,
    });

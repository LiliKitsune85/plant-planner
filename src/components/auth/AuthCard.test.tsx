...
describe('AuthCard', () => {
  ...
  it('renders footer node when provided and ignores footerLinks', () => {
    ...
    expect(
      screen.queryByRole('navigation', { name: /linki pomocnicze/i }),
  ...
  it('renders footerLinks when footer is omitted', () => {
    ...
    expect(
      screen.getByRole('navigation', { name: /linki pomocnicze/i }),
    ).toBeInTheDocument()
  ...
  it('omits footer entirely when footerLinks is empty', () => {
    const { container } = render(
      <AuthCard title="No footer" footerLinks={[]} children={baseChildren} />,
    )

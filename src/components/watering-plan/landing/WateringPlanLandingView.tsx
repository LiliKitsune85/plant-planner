...
export type WateringPlanLandingViewProps = {
  plantId: string
}
...
const buildStatusDescription = (context: CreatePlantResultSessionPayload | null) => {
  if (!context) {
    return {
      ...
      description:
        'Nie znaleziono kontekstu nowo utworzonej rośliny. Możesz kontynuować konfigurację planu ręcznie.',
      tone: 'default',
    } as const
  }
  ...
  switch (context.wateringSuggestion.status) {
    case 'available':
      return {
        ...
        description:
          'Możesz przejrzeć proponowany przez AI plan podlewania i w razie potrzeby go edytować.',
        tone: 'success',
      } as const
    case 'rate_limited':
      return {
...
export const WateringPlanLandingView = ({ plantId }: WateringPlanLandingViewProps) => {
  const [creationContext, setCreationContext] = useState<CreatePlantResultSessionPayload | null>(
    null,
  )

...
type MutationSuccess<TData = void> = { ok: true; data?: TData }
type MutationAborted = { ok: false; kind: 'aborted' }
type MutationFailure = { ok: false; kind: 'error'; error: PlantDetailMutationErrorVm }

export type PlantDetailMutationResult<TData = void> =
  | MutationSuccess<TData>
  | MutationAborted
  | MutationFailure

type UsePlantDetailMutationsParams = {
  plantId: string
}

export type UsePlantDetailMutationsResult = {
...
export const usePlantDetailMutations = ({
  plantId,
}: UsePlantDetailMutationsParams): UsePlantDetailMutationsResult => {
    ...
    async (note?: string | null): Promise<PlantDetailMutationResult> => {
      ...
      try {
        ...
        if (err instanceof WateringTaskApiError) {
          ...
          console.error('Unexpected error while creating adhoc watering entry', err)
  ...
  const deletePlant = useCallback(
    async (): Promise<PlantDetailMutationResult<DeletePlantResultDto>> => {
      deleteControllerRef.current?.abort()
      const controller = new AbortController()

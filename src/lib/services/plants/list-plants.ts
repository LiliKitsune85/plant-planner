...
const escapeIlikePattern = (value: string): string => value.replace(/[*]/g, "\\*");
...
const getSortValue = (row: PlantListRow, sort: PlantSortField): string => row[sort] ?? "";

const buildCursorFilter = (payload: ListPlantsCursorPayload, sort: PlantSortField): string => {
...
export const listPlants = async (
  ...
  let request = supabase.from("plants").select<PlantListRow>(PLANT_LIST_COLUMNS).eq("user_id", userId);

  if (search) {
    const pattern = `*${escapeIlikePattern(search)}*`;
    const encodedPattern = escapeLogicalValue(pattern);
    request = request.or([`species_name.ilike.${encodedPattern}`, `nickname.ilike.${encodedPattern}`].join(","));
  }
  ...
  if (error || !data) {
    logger.error("listPlants query failed", {
      error,
      userId,
    });

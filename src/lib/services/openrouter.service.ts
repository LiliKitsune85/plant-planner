...
export type OpenRouterChatMessage = { role: ChatRole; content: string }
...
export type OpenRouterChatJsonResponse<T> = {
  data: T
  model: string
  usage: TokenUsage
  latencyMs: number
...
export class OpenRouterService {
  ...
  async chat(request: OpenRouterChatRequest): Promise<OpenRouterChatResponse> {
    ...
    try {
      ...
      console.error('Unexpected OpenRouter chat error', { error })
  ...
  private async postChatCompletions(payload: ChatCompletionsPayload, signal: AbortSignal): Promise<OpenRouterResponseBody> {
    try {
      ...
      if (!response.ok) {
        ...
        console.error('OpenRouter responded with error', {
      ...
      try {
        ...
        console.error('Failed to parse OpenRouter success response', { error })
  ...
  private validate<T>(parsed: unknown, schema: ZodType<T>): T {
    ...
    if (!validation.success) {
      console.error('OpenRouter response schema validation failed', {

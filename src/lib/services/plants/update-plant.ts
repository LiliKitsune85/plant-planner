...
export interface UpdatePlantServiceCommand {
  plantId: string;
  userId: string;
...
const sanitizePayload = (payload: UpdatePlantCommand): UpdatePlantCommand =>
  Object.fromEntries(Object.entries(payload).filter(([, value]) => value !== undefined)) as UpdatePlantCommand;
...
export const updatePlant = async (
  ...
  if (error) {
    logger.error("updatePlant update failed", { error, plantId, userId });

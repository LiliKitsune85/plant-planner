...
const KNOWN_FORM_FIELDS: (keyof WateringPlanFormErrors)[] = [
  "form",
  "interval_days",
  "start_from",
  "custom_start_on",
...
const ensureErrorArray = (target: WateringPlanFormErrors, field: keyof WateringPlanFormErrors): string[] => {
  if (!target[field]) {
    target[field] = [];
  }
...
const appendFieldError = (target: WateringPlanFormErrors, field: keyof WateringPlanFormErrors, message: string) => {
...
const enumerateMonthsBetween = (start: string, end: string): string[] => {
  ...
  const normalizedStart = new Date(Date.UTC(startDate.getUTCFullYear(), startDate.getUTCMonth(), 1));
...
export const useWateringPlanMutations = () => {
  ...
  const submitSetPlan = useCallback(
    async (
      ...
      try {
        const { data, requestId: responseRequestId } = await setWateringPlan(plantId, command, options);
        setLastResult(data);

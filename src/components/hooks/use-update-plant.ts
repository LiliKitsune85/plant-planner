...
interface UseUpdatePlantArgs {
  plantId: string;
}

interface UpdatePlantSubmitOptions {
  baseFieldErrors?: PlantEditFormErrors["fields"];
}

interface UpdatePlantSuccessResult {
  ok: true;
  data: PlantDetailDto;
}

interface UpdatePlantAbortedResult {
  ok: false;
  kind: "aborted";
}

interface UpdatePlantFailureResult {
  ok: false;
  kind: "validation" | "api";
  formError: string;
...
export type UpdatePlantSubmitResult = UpdatePlantSuccessResult | UpdatePlantFailureResult | UpdatePlantAbortedResult;

export interface UseUpdatePlantResult {
  isSaving: boolean;
  submit: (payload: UpdatePlantCommand, options?: UpdatePlantSubmitOptions) => Promise<UpdatePlantSubmitResult>;
  cancel: () => void;
}
...
export const useUpdatePlant = ({ plantId }: UseUpdatePlantArgs): UseUpdatePlantResult => {
  ...
  const submit = useCallback(
    async (payload: UpdatePlantCommand, options: UpdatePlantSubmitOptions = {}): Promise<UpdatePlantSubmitResult> => {
      abortRef.current?.abort();
      ...
      try {
        ...
        logger.error("Unexpected error while updating plant", error);
        return {
          ok: false,
          kind: "api",

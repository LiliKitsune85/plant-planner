...
interface PlantsApiErrorOptions {
  status?: number;
  requestId?: string;
  details?: unknown;
...
interface RequestPlantsApiResult<TData> {
  response: Response;
...
export const listPlants = async (
  ...
  if (!response.ok || envelope.error || !envelope.data) {
    ...
    const message = errorPayload?.message ?? `Żądanie nie powiodło się (status ${response.status}).`;
...
interface GetPlantDetailParams {
  plantId: string;
}
...
export const getPlantDetail = async (
  ...
  const { response, envelope } = await requestPlantsApi<PlantDetailDto>(`/api/plants/${params.plantId}`, {
    method: "GET",
    signal: options.signal,
  });
  const requestId = detectRequestId(envelope.meta ?? null);
...
export const deletePlant = async (
  ...
  const { response, envelope } = await requestPlantsApi<DeletePlantResultDto>(`/api/plants/${plantId}?confirm=true`, {
    method: "DELETE",
    signal: options.signal,
  });
  const requestId = detectRequestId(envelope.meta ?? null);
